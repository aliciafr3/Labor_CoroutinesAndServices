...
fun ForecastWeatherView(forecast: List<ForecastItem>) {
    ...
    Column(
        ...
        if (searchQuery.value.isEmpty() && hometown.isEmpty()) {
            ...
            // Display forecast data using the WeatherCard for each item in the list
            LazyColumn(
                modifier = Modifier.fillMaxWidth()
            ) {
                // For every forecast item a WeatherCard is created that displays the data
                items(forecast) { forecastItem ->
                    WeatherCard(forecastItem = forecastItem) // a Weathercard is created with the correct data
                }
            }
        }
    }
